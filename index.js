...
app.post("/api/articles/new" ,async(req, res) => {
 ...
 let hasAllKeys = custUtils.checkifhaskeys(payload, ["name", "duration",
"age", "email", "title", "prof", "body"])
 if (!hasAllKeys) { res.send(errorViews.EntityMalformed("Required keys
missing", null)); return; }
 ...
 let oneInsert = await
client.db("main").collection("arts").insertOne({...payload, "_id": uuid(),
"timestamp": Date.now()});
...
app.get("/api/article", async(req, res) => {
 let {id} = req.query;
 let entities = await client.db("main").collection("arts").find({_id:
id}).toArray();
...
app.get("/api/articles", async(req, res) => {
 ...
 if ((!page) || (!size)) {
 res.send(errorViews.EntityMalformed("paramets missing! page and size
required!", null))
 return
 }
 let entities = await client.db("main").collection("arts").find({}).toArray();
...
// const uri =
"mongodb+srv://main:Sx4gpcPG0CEEaelq@maincluster.jjjnn.mongodb.net/main?retryWrites=t
rue&w=majority";